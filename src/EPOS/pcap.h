...
public:
    ...
    public:
        ...
        friend std::ostream & operator<<(std::ostream & db, const Global_Header & g) {
    ...
    {
    public:
        Packet_Header(uint64 timestamp, unsigned int len) :
            _ts_sec(timestamp / 1000000), _ts_usec(timestamp % 1000000),
            _incl_len(len), _orig_len(len) {}

        friend std::ostream & operator<<(std::ostream & db, const Packet_Header & p) {
