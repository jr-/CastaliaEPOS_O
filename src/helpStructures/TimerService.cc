...
{
    return timerDrift + resourceManager->getTemperatureDrift();
}

void TimerService::setResourceManager(ResourceManager * r)
{
    resourceManager = r;
}
...
void TimerService::recordTimerDrift()
{
    recordScalar("Timer drift", timerDrift);
}
...
simtime_t TimerService::getClock()
{
    ...
    lastRead += (now - lastReadTime) * getTimerDrift();
    lastReadTime = now;
    return lastRead;
}
