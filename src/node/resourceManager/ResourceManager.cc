...
double ResourceManager::getTemperature()
{
    return temperature;
}

void ResourceManager::setTemperature(double t)
{
    temperature = t;
}

double ResourceManager::getTemperatureDrift()
{
    if(temperatureModel.size() > 0) {
        auto entry = temperatureModel.lower_bound(temperature);
        if(entry->mean == 0 && entry->stdDev == 0)
            return 0;
        return normal(entry->mean, entry->stdDev);
    }
    return 0;
}
...
void ResourceManager::parseTemperatureModel()
{
    string fileName = par("temperatureModelFile");
    if(fileName.size() > 0) {
        std::ifstream file;
        ...
        while(file.good()) {
            double t, m, s;
            file >> t >> m >> s;
            temperatureModel.insert(TemperatureDrift(t,m,s));
        }
        file.close();
    }
}
...
void ResourceManager::finishSpecific()
{
	...
	recordScalar("Consumed Energy", initialEnergy - remainingEnergy);
	...
	if (getParentModule()->getIndex() == 0) {
		...
		recordScalar("Estimated network lifetime (days)", minLifetime);
		delete(topo);
	}
}
...
double ResourceManager::getCPUClockDrift(void)
{
	...
	trace()<< "cpuClockDrift: "<< cpuClockDrift << " (1.0f + cpuClockDrift): " << (1.0f + cpuClockDrift);
...
void ResourceManager::consumeEnergy(double amount)
{
	...
	if (remainingEnergy <= amount) {
		...
		recordScalar("Dead Node", SIMTIME_DBL(simTime()));
	} else
