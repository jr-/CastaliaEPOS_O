...
void Radio::initialize()
{
	...
	timeInState[TX] = 0;
	timeInState[SLEEP] = 0;
        lastEnterInState[RX] = 0;
        lastEnterInState[TX] = 0;
        lastEnterInState[SLEEP] = 0;
...
void Radio::finishSpecific()
{
	...
	recordScalar("Time in RX mode", timeInState[RX]);
	recordScalar("Time in TX mode", timeInState[TX]);
	recordScalar("Time in SLEEP mode", timeInState[SLEEP]);

	recordScalar("RX duty cycle", timeInState[RX] / simTime());
	recordScalar("TX duty cycle", timeInState[TX] / simTime());
	recordScalar("SLEEP duty cycle", timeInState[SLEEP] / simTime());
	recordScalar("RX+TX duty cycle", (timeInState[RX] + timeInState[TX]) / simTime());
...
double Radio::popAndSendToWirelessChannel()
{
	...
	if(doPcapLog) {
        ...
        /*
	if(pkt) {
	    unsigned char p0 = pkt->getData(0);
	    unsigned char p1 = pkt->getData(1);
	    unsigned short id = (p0 | (p1 << 8));
	    trace() << "Sending Packet, Transmission will last " << txTime << " secs. ID = " << id;
	}
	else
