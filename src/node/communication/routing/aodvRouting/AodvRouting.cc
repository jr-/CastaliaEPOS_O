...
void AodvRouting::setRreqBoardcastDropTimer(const char* dest, const char* source, int id){
	...
	rets.lifetime = simTime().dbl() + activeRouteTimeout/10;//0.6;
	...
	if(getTimer(AODV_RREQ_BROADCAST_DROP_TIMER).dbl()<=0){
		setTimer(AODV_RREQ_BROADCAST_DROP_TIMER, rreqBroadcast.top().lifetime - simTime().dbl());
	}
...
bool AodvRouting::checkExpireEntry(const iterable_priority_queue <RreqExpireTimerSet, vector<RreqExpireTimerSet>, RreqExpireLifetimeCompare>* exq,
								const char* dest){
...
void AodvRouting::cancelExpireEntry(iterable_priority_queue <RreqExpireTimerSet, vector<RreqExpireTimerSet>, RreqExpireLifetimeCompare>* exq,
								const char* dest){
    for(auto&& i : *exq) {
        if(i.dest.compare(dest) == 0) {
            i.canceled = true;
...
bool AodvRouting::isRreqRequstActive(const char* dest){
    ...
    for(const auto i : rreqBuffer)
        if(string(i->getDestinationAodv()).compare(dest)==0)
            return true;
//    {//on going
...
//        auto tmp = rreqBuffer;
//        while(!tmp.empty()) {
//            if(string(tmp.front()->getDestinationAodv()).compare(dest)==0)
//                return true;
//            tmp.pop();
//        }
//    }
...
//	    while(!tmp.empty()) {
//	        q0 = &tmp.top();
//	        if (!q0->canceled && q0->destination.compare(dest)==0)
//	            if (q0->lifetime > x)
//	                x = q0->lifetime;
//	        tmp.pop();
//	    }
...
//                                        RoutingTableEntryLifetimeCompare > tmp;
//	while(!rtExpireSeq.empty()) {
//	    q0 = rtExpireSeq.top();
//	    if(q0.destination.compare(dest)==0) {
//	        q0.canceled = true;
//	        tmp.push(q0);
//	        rtExpireSeq.pop();
//	        rtExpireSeq.swap(tmp);
//	        return;
//	    }
//	    tmp.push(q0);
//	    rtExpireSeq.pop();
...
void AodvRouting::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	...
	if(rt->isRouteValid(destination)){
		...
		return;
	        ...
	        trace() << "route NOT found in table";
...
void AodvRouting::processAodvRrepPackage(AodvRREPPacket * rrepPackage,int srcMacAddress, double rssi, double lqi){
	...
	if (!(string(rrepPackage->getOriginator()).compare(SELF_NETWORK_ADDRESS)==0)){
	    if(rrepPackage->getHopCount() < netDiameter) {
	        //forward the rrep
	...
	case AODV_RREQ_BLACKLIST_REMOVE_TIMER:
		{
			...
			if(!rreqBlacklistTimeout.empty()){
				...
				setTimer(AODV_RREQ_BLACKLIST_REMOVE_TIMER, t);
			}
		}
		break;
	default:
