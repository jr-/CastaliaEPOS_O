...
if [[ $# < 2 ]]; then
    echo "Error! Usage: $0 <ini file> <label>"
    exit 1
fi
...
#for config in BypassRouting LEACH AODVT LABILE REL AODV TSTP; do
for config in EUCLIDEAN EFFORT TTL; do
    echo "Running config $config"
    echo "Castalia --config=$config --input=\"$INI\" --output=\"$OUTPUTS/$config-castalia-output.txt\""
    SECONDS=0
    Castalia --config=$config --input="$INI" --output="$OUTPUTS/$config-castalia-output.txt"
